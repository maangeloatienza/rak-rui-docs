<template>
  ...
  >
    ...
    <div
      v-for="(sidebarGroupItem, index) in currentSidebar"
      :key="sidebarGroupItem.to || index"
      class="group"
    >
      <div class="group__title">
        {{ sidebarGroupItem.title }}
      </div>
      <div class="group__body">
...
export default {
  ...
  computed: {
    ...
    localePathList() {
      return Object.keys(this.$site.locales || {}).map((locale) => {
        ...
        if (item.path === this.$localePath) {
          ...
          path = this.$page.path.replace(this.$localePath, item.path) 
    ...
    currentSidebar(){
      let sidebars = this.sidebars;
      let currentPath = this.currentPagePath;
      let tempHolder = []
      sidebars.map(sidebar=>{
        if(currentPath.includes(sidebar.to)){
          tempHolder.push(sidebar)
        }
      })
